  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      body: AsyncValueWidget<ActivityDetail>(
        ...
        data: (activity) => _ActivityDetailBody(activityId: activityId, activity: activity),
      ),
    );
  }
}
...
class _ActivityDetailBody extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_ActivityDetailBody> createState() => _ActivityDetailBodyState();
}
...
class _ActivityDetailBodyState extends ConsumerState<_ActivityDetailBody> {
  ...
  Future<void> _handleJoin() async {
    await ref.read(requestActionsControllerProvider.notifier).join(
          widget.activityId,
    ...
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Katılım isteği gönderildi!')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isParticipant = currentUserId != null && activity.participants.any((p) => p.userId == currentUserId);
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    if (isCancelled)
                      const _InfoChip(icon: Icons.cancel_outlined, label: 'İptal edildi', tone: _ChipTone.muted)
                    else if (isFull)
                      const _InfoChip(icon: Icons.people_outline, label: 'Dolu', tone: _ChipTone.muted)
                    else
                      const _InfoChip(icon: Icons.people_outline, label: 'Açık', tone: _ChipTone.success),
                  ],
                ),
                const SizedBox(height: 10),
                Text(activity.title, style: const TextStyle(fontSize: 21, fontWeight: FontWeight.w700)),
                const SizedBox(height: 10),
                Row(
                  children: [
                    const Icon(Icons.calendar_today, size: 14, color: AppColors.accent),
                    const SizedBox(width: 6),
                    Text(Formatters.eventDate(activity.eventDate), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.accent)),
                  ],
                ),
                ...
                Row(
                  children: [
                    if (activity.participants.isNotEmpty)
                      SizedBox(
                        width: 26.0 + (activity.participants.length.clamp(0, 4) - 1) * 18.0,
                        height: 26,
                        child: Stack(
                          children: [
                            for (var i = 0; i < activity.participants.take(4).length; i++)
                              Positioned(
                                left: i * 18.0,
                                child: Container(
                                  decoration: const BoxDecoration(shape: BoxShape.circle, border: Border.fromBorderSide(BorderSide(color: AppColors.background, width: 2))),
                                  child: UserAvatar(
                                    displayName: activity.participants[i].displayName,
                                    avatarUrl: activity.participants[i].avatarUrl,
                                    size: 26,
                                  ),
                                ),
                              ),
                    ...
                    Text('${activity.currentPeopleCount} / $totalSpots katılıyor', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
                  ],
                ),
                const SizedBox(height: 10),
                Wrap(
                  spacing: 6,
                  children: [
                    _InfoChip(icon: Icons.emoji_events_outlined, label: _skillLabels[activity.skillLevel]!),
                    _InfoChip(icon: Icons.people_outline, label: _genderLabels[activity.genderPreference]!),
                  ],
                ),
                if (activity.description != null && activity.description!.isNotEmpty) ...[
                  const SizedBox(height: 16),
                  const _SectionTitle('Hakkında'),
                  Text(activity.description!, style: const TextStyle(fontSize: 13, color: Color(0xFF374151), height: 1.5)),
                ],
                const SizedBox(height: 16),
                ...
                GridView.count(
                  ...
                  children: [
                    _DetailBox(
                      ...
                      value: (activity.pricePerPerson ?? 0) > 0 ? '${activity.pricePerPerson} ₺' : 'Ücretsiz',
                    ),
                    _DetailBox(icon: Icons.people_outline, label: 'Kontenjan', value: '$totalSpots kişi'),
                    _DetailBox(icon: Icons.groups_outlined, label: 'Şu an', value: '${activity.currentPeopleCount} kişi'),
                    _DetailBox(icon: Icons.emoji_events_outlined, label: 'Seviye', value: _skillLabels[activity.skillLevel]!),
                  ],
                ),
                const SizedBox(height: 16),
                ...
                if (activity.addressDetailPrivate != null && activity.addressDetailPrivate!.isNotEmpty) ...[
                  const SizedBox(height: 16),
                  const _SectionTitle('Konum'),
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
                    decoration: BoxDecoration(color: const Color(0xFFF9FAFB), borderRadius: BorderRadius.circular(AppRadius.lg)),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Row(
                          children: [
                            const Icon(Icons.vpn_key_outlined, size: 12, color: AppColors.success),
                            const SizedBox(width: 5),
                            Text(activity.addressDetailPrivate!, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.success)),
                          ],
                        ),
                ...
                if (activity.participants.isNotEmpty) ...[
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Text('${activity.participants.length} kişi', style: const TextStyle(fontSize: 12, color: AppColors.mutedForeground)),
                    ],
                  ),
                ...
                const SizedBox(height: 16),
                if (!isCancelled && !isFull && !isParticipant) _JoinCard(
                ...
                if (isFull && !isCancelled)
                  Container(
                    ...
                    child: const Text('Kontenjan doldu', textAlign: TextAlign.center, style: TextStyle(color: AppColors.mutedForeground)),
                  ),
                if (isCancelled)
                  Container(
                    width: double.infinity,
                    padding: const EdgeInsets.symmetric(vertical: 14),
                    margin: const EdgeInsets.only(bottom: 12),
                    decoration: BoxDecoration(color: AppColors.destructiveBg, borderRadius: BorderRadius.circular(AppRadius.md)),
                    child: const Text('Bu etkinlik iptal edildi', textAlign: TextAlign.center, style: TextStyle(color: AppColors.destructive)),
                  ),
          ...
          ? Column(
              children: [
                TextField(
                  ...
                  maxLines: 3,
                  decoration: const InputDecoration(hintText: 'Kendinizi tanıtın (opsiyonel)...'),
                ),
                const SizedBox(height: 10),
                Row(
                  children: [
                    Expanded(
                      child: ElevatedButton(onPressed: onSubmit, child: const Text('İstek Gönder')),
                    ),
                    const SizedBox(width: 8),
...
class _DetailBox extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
      decoration: BoxDecoration(color: const Color(0xFFF9FAFB), borderRadius: BorderRadius.circular(AppRadius.md)),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
...
class _InfoChip extends StatelessWidget {
  const _InfoChip({required this.icon, required this.label, this.tone = _ChipTone.none});
  ...
  Widget build(BuildContext context) {
    ...
    final background = tone == _ChipTone.success ? AppColors.successBg : const Color(0xFFF3F4F6);
    return Container(
